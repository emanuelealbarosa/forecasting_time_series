...
class method():
    ...
    def predict(self, values):
        ...
        if self.check_length_ok(values):
            try:
                ...
                X = X.reshape((X.shape[0], self.number_subsequences, 1, self.subsequence_length, 1))
                ...
                X_input = values[-self.number_input_lags:].reshape(
                    1, self.number_subsequences, 1, self.subsequence_length, 1)
    ...
    def build_model(self):
        model = Sequential()
        model.add(ConvLSTM2D(filters=self.number_filters, kernel_size=(1, self.kernel_size), activation='relu',
                              input_shape=(self.number_subsequences, 1, self.subsequence_length, 1)))
        model.add(Flatten())
        model.add(Dense(1))
