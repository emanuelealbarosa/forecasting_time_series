...
class method():
    ...
    def check_length_ok(self, values):
        return len(values) > self.number_input_lags and self.number_input_lags >= self.kernel_size
    ...
    def build_model(self):
        model = Sequential()
        model.add(Conv1D(filters=self.number_filters, kernel_size=self.kernel_size, activation='relu',
                          input_shape=(self.number_input_lags, 1)))
        pool_size = 2 if (self.number_input_lags - self.kernel_size + 1) >= 2 else 1
