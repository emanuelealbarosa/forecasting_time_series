...
class method():
    ...
    def __init__(self, method_parameters):
        ...
        self.number_hidden_nodes = method_parameters.get('number_hidden_nodes')
    ...
    def predict(self, values):
        ...
        if self.check_length_ok(values):
            try:
                ...
                X_input = values[-self.number_input_lags:].reshape(1, self.number_input_lags)
    ...
    def build_model(self):
        model = Sequential()
        model.add(Dense(self.number_hidden_nodes, activation='relu', input_shape=(self.number_input_lags,)))
        model.add(Dense(1))
